// Generated from c:/Users/juanm/OneDrive/Escritorio/Analizador nuevo/Calculator.g4 by ANTLR 4.13.2
// jshint ignore: start
import antlr4 from 'antlr4';

// This class defines a complete listener for a parse tree produced by CalculatorParser.
export default class CalculatorListener extends antlr4.tree.ParseTreeListener {

	// Enter a parse tree produced by CalculatorParser#program.
	enterProgram(ctx) {
	}

	// Exit a parse tree produced by CalculatorParser#program.
	exitProgram(ctx) {
	}


	// Enter a parse tree produced by CalculatorParser#action.
	enterAction(ctx) {
	}

	// Exit a parse tree produced by CalculatorParser#action.
	exitAction(ctx) {
	}


	// Enter a parse tree produced by CalculatorParser#comando.
	enterComando(ctx) {
	}

	// Exit a parse tree produced by CalculatorParser#comando.
	exitComando(ctx) {
	}



}